...
default_scavenger_weighting_func()
{
	num_to_add = 1;

	if( isDefined( level.pulls_since_last_scavenger ) && level.round_number < 100 )		// --JC / Demzyy ;D <3
	{
		// after 25 pulls the percentage increases to 50%
		if( level.pulls_since_last_scavenger > 25 )
		{
			num_to_add += 0.5;
		}
		// after 20 pulls the percentage increases to 40%
		else if( level.pulls_since_last_scavenger > 20 )
		{
			num_to_add += 0.4;
		}
		// after 15 pulls the percentage increases to 30%
		else if( level.pulls_since_last_scavenger > 15 )
		{
			num_to_add += 0.3;
		}
		// after 10 pulls the percentage increases to 20%
		else if( level.pulls_since_last_scavenger > 10 )
		{
			num_to_add += 0.2;
		}
		// after 5 pulls the percentage increases to 10%
		else if( level.pulls_since_last_scavenger > 5 )
		{
			num_to_add += 0.1;
		}
	}
...
init_weapons()
{
	...
	add_zombie_weapon( "colt45_mp", 							&"ZOMBIE_WEAPON_COLT_50", 					50,		"vox_crappy",	8 );
	add_zombie_weapon( "cz75", 							&"ZOMBIE_WEAPON_COLT_50", 					50,		"vox_crappy",	8 );
	add_zombie_weapon( "python", 							&"ZOMBIE_WEAPON_COLT_50", 					50,		"vox_357",	8 );
	...
	//add_zombie_weapon( "remington700_mp", 					    &"ZOMBIE_WEAPON_KAR98K_200", 				200,	"",				0);
	...
	add_zombie_weapon( "m14", 								 "Press and Hold &&1 to buy M14 [Cost: 500]", 	500,	"" ,			0 );
	add_zombie_weapon( "m16", 								 "Press and Hold &&1 to buy M16 [Cost: 1500]", 	1500,	"vox_ppsh" ,			5 );
	add_zombie_weapon( "fal", 								 "Press and Hold &&1 to buy M14 [Cost: 500]", 	500,	"vox_crappy" ,			0 );
	...
	add_zombie_weapon( "stielhandgranate", 						&"ZOMBIE_WEAPON_FRAGGRENADE_250", 		250,	"" ,			0, 250 );
	...
	add_zombie_weapon( "dragunov", 	&"ZOMBIE_WEAPON_TYPE99_S_B_750", 			750,	"",		5);
	add_zombie_weapon( "l96a1", 	&"ZOMBIE_WEAPON_TYPE99_S_B_750", 			750,	"",		5);
	...
	add_zombie_weapon( "mpl", 					"Press and Hold &&1 to buy MPL [Cost: 1000]", 				1000,	"",				0 );
	add_zombie_weapon( "pm63", 					"Press and Hold &&1 to buy PM63 [Cost: 1000]", 				1000,	"",				0 );
	add_zombie_weapon( "mp5", 					"Press and Hold &&1 to buy MP5K [Cost: 1000]", 				1000,	"",				0 );
	add_zombie_weapon( "ak74u", 					"Press and Hold &&1 to buy AK74U [Cost: 1200]", 				1200,	"",				0 );
	add_zombie_weapon( "aug", 					"Press and Hold &&1 to buy AK74U [Cost: 1200]", 				1200,	"",				0 );
	add_zombie_weapon( "commando", 					"Press and Hold &&1 to buy AK74U [Cost: 1200]", 				1200,	"",				0 );
	add_zombie_weapon( "famas", 					"Press and Hold &&1 to buy AK74U [Cost: 1200]", 				1200,	"",				0 );
	add_zombie_weapon( "g11", 					"Press and Hold &&1 to buy AK74U [Cost: 1200]", 				1200,	"",				0 );
	add_zombie_weapon( "galil", 					"Press and Hold &&1 to buy AK74U [Cost: 1200]", 				1200,	"vox_ppsh",				0 );
	add_zombie_weapon( "spectre", 					"Press and Hold &&1 to buy AK74U [Cost: 1200]", 				1200,	"",				0 );
	...
	add_zombie_weapon( "olympia", 				                "Press and Hold &&1 to buy Olympia [Cost: 500]", 500,	"vox_shotgun", 6);
	add_zombie_weapon( "stakeout", 				                "Press and Hold &&1 to buy Stakeout [Cost: 1500]", 1500,	"vox_shotgun", 6);
	add_zombie_weapon( "hs10", 				                "Press and Hold &&1 to buy Stakeout [Cost: 1500]", 1500,	"vox_shotgun", 6);
	add_zombie_weapon( "spas", 				                "Press and Hold &&1 to buy Stakeout [Cost: 1500]", 1500,	"vox_shotgun", 6);
	...
	//add_zombie_weapon( "dp28_mp", 								&"ZOMBIE_WEAPON_DP28_2250", 				2250,	"vox_mg" ,		9 );
	add_zombie_weapon( "hk21", 								&"ZOMBIE_WEAPON_DP28_2250", 				2250,	"vox_mg" ,		9 );
	add_zombie_weapon( "rpk", 								&"ZOMBIE_WEAPON_DP28_2250", 				2250,	"vox_mg" ,		9 );
	...
	add_zombie_weapon( "china_lake", 			&"ZOMBIE_WEAPON_PANZERSCHREK_2000", 		2000,	"vox_panzer",	5 );
	add_zombie_weapon( "m72_law", 			&"ZOMBIE_WEAPON_PANZERSCHREK_2000", 		2000,	"vox_panzer",	5 );
	...
	add_zombie_weapon( "ray_gun", 								"Press and Hold &&1 to buy Ray Gun [Cost: 5000]", 				5000,	"vox_raygun",	6 );
	...
	//add_zombie_weapon( "type99lmg_bayonet_mp",					&"ZOMBIE_BUY_TESLA", 						10,		"",	5 );
	add_zombie_weapon( "thundergun", 								&"ZOMBIE_WEAPON_RAYGUN_10000", 				10000,	"vox_raygun",	6 );
	add_zombie_weapon( "scavenger", 								&"ZOMBIE_WEAPON_RAYGUN_10000", 				10000,	"vox_ppsh",	6 );
	...
	add_limited_weapon( "thundergun", 1);
...
treasure_chest_init()
{
	...
	if (level.chests.size > 1)
	{
		...
		if(level.script != "nazi_zombie_prototype")
		{
			...
			level.boxhits = 0;		// For box hit counter hud element.  Using this instead of "chest_accessed" to show WHEN the box is purchased, rather than when the weapon dissappears  --Just Crusader / JC
		}
...
{	
	...
	while( 1 )
	{
		...
		if( is_player_valid( user ) && user.score >= cost )
		{
			...
			level.boxhits += 1;		// For box hit counter hud element  --Just Crusader / JC
			break; 
		}
	...
	else
	{
		...
		while( 1 )
		{
			...
			if( grabber == user || grabber == level )
			{
				...
				if( grabber == user && is_player_valid( user ) && user GetCurrentWeapon() != "mine_bouncing_betty" && user GetCurrentWeapon() != "zombie_perk_bottle_doubletap"
				&& user GetCurrentWeapon() != "zombie_perk_bottle_jugg" && user GetCurrentWeapon() != "zombie_perk_bottle_revive" && user GetCurrentWeapon() != "zombie_perk_bottle_sleight"
				&& user GetCurrentWeapon() != "zombie_bowie_flourish" && user GetCurrentWeapon() != "zombie_perk_bottle_mulekick" && user GetCurrentWeapon() != "zombie_perk_bottle_phd"
				&& user GetCurrentWeapon() != "zombie_perk_bottle_staminup" && user GetCurrentWeapon() != "zombie_perk_bottle_phd" )
				{
		...
		//increase counter of amount of times a  weapon grabbed.
		if(level.script != "nazi_zombie_prototype")
		{
		level.chest_accessed += 1;
		
			// Wonder Weapon Weighting Functions -Demzyy
			...
			if( isDefined(level.pulls_since_last_scavenger) )
			{
				level.pulls_since_last_scavenger += 1;
			}
...
treasure_chest_lid_open()
{
	...
	self RotateRoll( 105, openTime, ( openTime * 0.4 ) ); //0.5
...
treasure_chest_lid_close( timedOut )
{
	...
	self RotateRoll( closeRoll, closeTime, ( closeTime * 0.4 ) ); //0.4
...
treasure_chest_weapon_spawn( chest, player )
{
	...
	else
	{
		...
		if( rand == "tesla_gun" || rand == "ray_gun" || rand == "thundergun" || rand == "scavenger" )
		{
			// Wonder Weapon Weighting Functions -Demzyy 
			if(isDefined(level.DLC3.useWeaponSpawn) && level.DLC3.useWeaponSpawn ) 
			{
				...
				if( rand == "scavenger" )
				{
					...
					level.player_seen_scavenger = true;
				}
			}
			else
			{
		...
		if( isdefined( current_weapon ) )
		{
			if( !( weapon_string == "fraggrenade" || weapon_string == "stielhandgranate" || weapon_string == "molotov" || weapon_string == "zombie_cymbal_monkey" ) )
			{
				...
				if( isDefined(level.DLC3.useGiveWeapon) && level.DLC3.useGiveWeapon)
				{
					...
					if( current_weapon == "thundergun" )
					{
						level.player_drops_thundergun = true;
					}

					if( current_weapon == "scavenger" )
					{
						level.player_drops_scavenger = true;
					}
		...
		for( i = 0; i < primaryWeapons.size; i++ )
		{
			...
			if( weapon_string != "fraggrenade" && weapon_string != "stielhandgranate" && weapon_string != "molotov" && weapon_string != "zombie_cymbal_monkey" )
			{
				...
				if( isDefined(level.DLC3.useGiveWeapon) && level.DLC3.useGiveWeapon )
				{
					...
					if( primaryWeapons[i] == "thundergun" )
					{
						level.player_drops_thundergun = true;
					}

					if( primaryWeapons[i] == "scavenger" )
					{
						level.player_drops_scavenger = true;
					}

...
init()
{
	...
	set_zombie_var( "zombie_perk_juggernaut_health",	260);
...
turn_revive_on()
{
	...
	if(players.size == 1)
		solo_trig SetHintString( "Press & hold &&1 to buy Quick Revive [Cost: 500]" );
	/*else
		level waittill("revive_on");*/  // we don't want it waiting for power, even in co-op, on this map, for first room challenges  --Just Crusader / JC
...
turn_doubletap_on()
{
	...
	for( i = 0; i < machine.size; i++ )
	{
		machine[i] setmodel("bo2_zombie_vending_doubletap2_on");
		machine[i] vibrate((0,-100,0), 0.3, 0.4, 3);
		machine[i] playsound("perks_power_on");
...
vending_trigger_think()
{
	...
	for( ;; )
	{
		...
		case "specialty_quickrevive":
			...
			{
				cost = 500;
				//self.reviveUsesLeft--;  This was causing the purchase revive use issue  --Just Crusader / JC
			}
			else
			cost = 1500;
			break;
		...
		if( (perk == "specialty_quickrevive") && ( !is_coop() ) )	// Updated revive purchase and removal from JBird632's Overrun --Just Crusader / JC
		{
			self.reviveUsesLeft--;
			if( self.reviveUsesLeft == 0 )
			{
				self remove_revive();
			}
		}
...
{
		self endon( "death" );		// added by Just Crusader / JC
		dist = 128 * 128;
		while(true)
		{
			players = get_players();
...
vending_set_hintstring( perk )
{
	...
	case "specialty_quickrevive":
		...
		if(players.size == 1)
		{
			...
			if(self.reviveUsesLeft == 0)
				{
					self SetHintString( "Out of Stock" );
				}
	...
	case "specialty_rof":
		self SetHintString( "Press & hold &&1 to buy Double Tap 2.0 [Cost: 2000]" );
		break;
...
remove_revive()		// JBird632 script from Overrun --Just Crusader / JC
{
	revive_machine = GetEnt("vending_revive", "targetname");
	revive_trig = GetEnt("quick_revive_trig", "targetname");
	revive_machine_clip = GetEnt("quick_revive_clip", "targetname");
	revive_bump = GetEnt( "revive_bump_trigger", "targetname");
	revive_sound = GetEnt( "perksacola_revive", "targetname");
	players = GetPlayers();
	
	revive_trig delete();
	revive_bump delete();
	revive_sound delete();
	level.temp_org_revive delete();
	
	for(i=0;i<players.size;i++)
		players[i] playsound("laugh_child");
		
	revive_machine Vibrate( (50, 0, 0), 5, 0.5, 5 );
	revive_machine MoveZ( 35, 5 );
	revive_machine waittill("movedone");
	
	playfx(level._effect["poltergeist"], revive_machine.origin);
	playsoundatposition ("box_poof", revive_machine.origin);
	wait(0.1);
	revive_machine delete();
	revive_machine_clip delete();
}

...
// set_zombie_var( "zombie_spawn_delay"
...
main(init_zombie_spawner_name)
{
	...
	// Limit zombie to 24 max, must have for network purposes;    "23" actually equals 24 in-game --JC
	SetAILimit( 23 );
	...
	#/
	// level thread maps\_zombiemode_perk_whoswho::whoswho_init();
	init_utility();
...
//	level thread coop_player_spawn_placement();
	...
	// Custom threads
	level thread timer_hud(); //5and5
	level thread round_timer_hud(); //5and5
	level thread fake_reset(); //5and5
	level thread boxhits_hud();	//5and5 and Just Crusader / JC
...
#/
	...
	set_zombie_var( "penalty_no_revive_percent", 		30, 	100 );
	set_zombie_var( "penalty_died_percent", 			0, 		100 );
	set_zombie_var( "penalty_downed_percent", 			50, 		100 );	
...
onPlayerConnect_clientDvars()
{
	self SetClientDvars(
		...
		"cg_drawfps", "2",
		"hud_fade_ammodisplay", "0",		// ammo on HUD never fades away
		"cg_blood", "0",
		"cg_mature", "0"
		*/		// These are repeated in nazi_zombie_wunder
		 );
...
set_third_person( value )
{
	if( value )
	{
		self SetClientDvars( "cg_thirdPerson", "1",
			"cg_fov", "55",		// "40" be default; all other mentions of fov are "65" be default
...
round_spawning()
{
	...
	level endon( "end_of_round" );		// JC endon added from BO1: also added new reference to this in "round_think()"  --Just Crusader / JC
	//level endon( "restart_round" );	// BO1 adds this, but I don't think we need it here because there's no WaW reference to it that I know of  --Just Crusader / JC
/#
...
#/
	...
	while( count < max )
	{
		...
		while( get_enemy_count() > 31 || level.zombie_total <= 0 )		// changed to match BO1 and moved to start of thread, as in BO1  --Just Crusader / JC
		{
			wait( 0.1 );
		}
	...
	//	iPrintLn(spawn_point.targetname + " " + level.zombie_vars["zombie_spawn_delay"]);
		/*while( get_enemy_count() > 31 )		// changed to match BO1 and moved to start of thread, as in BO1  --JC
		{
			wait( 0.05 );
		}*/
...
{	
	...
	for( ;; )
	{
		...
		/*if( level.first_round == true && level.round_number != 1 )	// Spawn and sprint rate function for test games and first room version  --Just Crusader / JC
		{
			timer = 2;		// this should be default level.zombie_vars["zombie_spawn_delay"]; for nearly every map
			
			calc_loop_number = level.round_number - 1;

			level.zombie_move_speed = calc_loop_number * 8;
			//iprintlnbold(level.zombie_move_speed);

			//timer = timer * ( 0.95 ^ calc_loop_number );
			
			while( calc_loop_number >= 1 )
			{
				timer = ( timer * 0.95 );
			...
			//iprintlnbold(level.zombie_vars["zombie_spawn_delay"]);
		}*/

		// Replacement for Spawn Delay and Zombie Movement Speed Increase functions, allows for correct calculations at start of first room and test games  --Just Crusader / JC
		level.zombie_move_speed = (level.round_number - 1) * 8;

		if( level.zombie_move_speed < 1 )
		{
		...
		if( level.first_round )
		{
			level.first_round_spawnrate = level.zombie_vars["zombie_spawn_delay"];		// this should be "2" by for nearly every map, but using this variable lets you change it if you like
		}

		timer = level.first_round_spawnrate;

		for ( i=2; i<=level.round_number; i++ )		// same as level.round_number - 1, and will not run on round 1, which is exactly what we need here
		{
			timer = ( timer * 0.95 );
		}
		...
		level notify( "start_of_round" );		// BO1 adds this, but I don't think we need it here because there's no WaW reference to it that I know of  --Just Crusader / JC
												// I use it for my round timer thanks JC <3 -5and5
		round_wait(); 
		level.first_round = false;
		level notify( "end_of_round" );		// need this for new "end_of_round" flag to work in "round_spawning" thread  --Just Crusader / JC
...
{	
	...
	if(level.round_number >= 99 && level.round_number % 2 == 1)
	{
		level.zombie_health = 15;
		return;
	}
	
	//Demzyy is a GREAT scripter!  --JC ;D
	
	if(level.round_number > 99 && level.round_number % 2 == 0)
	{
		level.zombie_health = 10000000;
		return;
	} 
	...
	{
		// Until round 10, only increase by 100 HP each round
		if(i <= 10)
		{
		...
		{
			level.zombie_health = 125000;
		}

		else if( i > 62 && i <= 65 )
		{
			level.zombie_health = 175000;
		}

		else if( i > 65 && i <= 70 ) 
		{
			level.zombie_health = 250000;	
		}

		else if( i > 70 && i < 99 )
		{
			level.zombie_health = 300000;
		}
		/*
		else
		{
...
round_spawn_failsafe()
{
	...
	while(1)
	{
		...
		{
			// DEBUG HACK
			self.marked_for_death = true;		// This will prevent being able to get drops from bleedouts  --Just_Crusader / JC
			self dodamage( self.health + 100, (0,0,0) );	
			break;
		}
...
end_game()
{
	...
	if (level.win_game) //5and5
	{
		...
		for(i=0;i<players.size;i++)
		{
        	...
        	//players[i] SetClientDVar("snd_menu_sfx",1);
        	//players[i] SetClientDVar("snd_menu_voice",1);
		}
		game_over SetText( "THERE IT IS! THERE IT IS!" );
    ...
    if( level.tom_victory == true )
    {
        ...
        survived SetText( "You wasted " , level.round_number , " hours of your life before escaping!" );
    }
...
intermission()
{
	...
	for( i = 0; i < players.size; i++ )
	{
		...
		players[i].health = 130; // This is needed so the player view doesn't get stuck
...
{	
	...
	timer SetTimerUp(0);
	level thread coop_pause(timer);

	start_time = int(getTime() / 1000);
	level.paused_time = 0;

	// pause timer once game ends
	level waittill ( "end_game" );
	while (1)
	{	
...
boxhits_hud() //5and5 and Just Crusader / JC
{
	boxhits = NewHudElem();
	boxhits.horzAlign = "left";
	boxhits.vertAlign = "top";
	boxhits.alignX = "left";
	boxhits.alignY = "top";
	boxhits.y += 16;
	boxhits.x += 5;
	boxhits.fontScale = 1.3;
	boxhits.alpha = 1;
	boxhits.hidewheninmenu = 0;
	boxhits.foreground = 1;
	boxhits.label = "Box Hits: "; 
	boxhits.color = ( 1.0, 1.0, 1.0 );

	while(1)
	{
		if(getDvarInt( "hud_box" ) == 0)
		{
			if(boxhits.alpha != 0)
			{
				boxhits.alpha = 0;
		...
		else
		{
			...
			{
				boxhits.alpha = 1;
				//boxhits setText("Box Hits: ", level.boxhits); // setText errors after too many different strings are entered
				boxhits setValue( level.boxhits ); // setValue does not error
			}
		}
		wait 0.05;
	}
...
round_timer(hud)
{
	...
	{	
		...
		time = end_time - start_time - 0.1; // need to set time below the number or it will show the next number
		hud.label = "Round Time: ";
		level thread display_times(hud, time);

		level waittill( "start_of_round" );

		wait 1; // to line up with the number displaying
...
{	
	start_time = int(getTime() / 1000 + 1); // +1 to fix rounding issue
	level.paused_time = 0;
	paused_start_time = 0;
	paused = false;
...
fake_reset() //5and5
{
    ...
    {
        current_time = int(getTime() / 1000);
        //level.total_time = current_time - start_time; use if coop_pause removed
		level.total_time = current_time - level.paused_time - start_time; //use if coop_pause implemented

...
get_valid_powerup()		// August 2020 Update  --Just Crusader / JC
{
	...
	while( 1 )
	{
		...
		else if( powerup == "firesale" && ( level.box_moved == false || level.box_moved != true ) )
		{
			powerup = get_next_powerup();
		}
		else if( powerup == "perk" )
		{
			powerup = get_next_powerup();
		}
		else
...
watch_for_drop()
{
	...
	while (1)
	{
		//flag_wait( "zombie_drop_powerups" ); removed because incompat
...
/*powerup_drop(drop_point)
{
	...
	//level.powerup_drop_count++; moved
...
//new remix powerup_drop(drop_point)
powerup_drop(drop_point)
{
	if( level.powerup_drop_count >= level.zombie_vars["zombie_powerup_drop_max_per_round"] )
	{
...
#/
	...
	level.powerup_drop_count++; //moved to
	...
	//level.powerup_drop_count++; moved from
...
powerup_setup()
{
	powerup = get_valid_powerup();		// August 2020 Update  --Just Crusader / JC
...
// kill them all!
nuke_powerup( drop_item )		// Nuke script from JBird632's Overrun  --Just Crusader / JC
{
	...
	//array_thread( zombies,::NukeThem ); // this is the cool stand-still anim for being nuked
	...
	for (i = 0; i < zombies.size; i++)
	{
		...
		zombies[i] dodamage( zombies[i].health + 555, zombies[i].origin );
...
check_for_instakill( player )
{
	if( IsDefined( player ) && IsAlive( player ) && level.zombie_vars["zombie_insta_kill"])
	{
		...
		if( flag( "dog_round" ) )
		{
			self DoDamage( self.health + 555, self.origin, player );
...
// This here plays kinda a cool animation for when zombies are being nuked, but forces them to stand still
//NukeThem(){
//	self endon( "death" );
//	while(1){

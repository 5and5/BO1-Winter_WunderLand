...
black_hole_on_player_spawned( int_local_client_num )
{
	...
	if ( IsDefined( self._zombie_visionset_list ) )
	{
		...
		{
			if ( IsDefined( self._zombie_visionset_list[i].vision_set ) && self._zombie_visionset_list[i].vision_set == level._visionset_black_hole_bomb )
			{
				self zombie_vision_set_remove( level._visionset_black_hole_bomb, 0, int_local_client_num );
			}
		}
	}
}
